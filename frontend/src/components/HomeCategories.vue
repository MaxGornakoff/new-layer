...
<template>
  <section id="categories">
    <h2 class="text-3xl font-semibold uppercase my-10">Категории пластика</h2>
    <AppLoader v-if="loading" label="Загрузка категорий..." />
    <div v-else-if="categories.length" class="flex gap-5 max-md:flex-col justify-between">
      <RouterLink
        v-for="category in categories"
        :key="category.id"
        :to="`/catalog?category=${category.slug}`"
        class="relative flex min-h-[280px] flex-1 items-stretch overflow-hidden rounded-[20px] text-white no-underline transition duration-200 ease-out hover:-translate-y-1 hover:shadow-[0_12px_32px_rgba(15,23,42,0.12)] max-md:min-h-0"
      ...
      >
        ...
        <div class="z-4 flex flex-col items-start justify-between p-7">
          <ul
            v-if="categoryAdvantages(category).length"
            class="m-0 flex list-none flex-col gap-[15px] p-0"
          >
            <li
              v-for="(advantage, index) in categoryAdvantages(category)"
              :key="index"
              class="flex items-center gap-3.5 text-[15px] leading-[1.2]"
            >
              <img
          ...
          <span
            class="inline-flex rounded-full bg-white px-7.5 py-2 text-base text-[#222222]"
          >
        ...
        <div
          ...
          class="pointer-events-none top-[17%] right-[-20%] absolute z-3 flex h-[330px] max-w-[280px] items-end justify-center max-md:relative max-md:mx-auto max-md:h-auto max-md:max-h-[180px] max-md:w-full max-md:max-w-none max-md:px-4 max-md:pb-4"
        >
          <img
